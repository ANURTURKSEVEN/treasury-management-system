...
/** Bir para piyasası borçlanma deal'inin detayı: özet + SWIFT (MT320/MT202) önizleme. */
public class MoneyMarketDetailDialog extends JDialog {

    private final MoneyMarketBorrowing d;

    public MoneyMarketDetailDialog(Window owner, MoneyMarketBorrowing d) {
        super(owner, "Para Piyasası Deal — " + d.getReferenceNo(), ModalityType.APPLICATION_MODAL);
        this.d = d;
    ...
    private JComponent buildSummary() {
        ...
        st.setForeground("ACTIVE".equals(d.getStatus()) ? new Color(0x1E8E3E)
                : "CANCELLED".equals(d.getStatus()) ? new Color(0xC5221F) : new Color(0x374151));
        ...
        row = kv(card, g, row, "Geri Ödeme:", money(d.getRepaymentAmount()));
    ...
    private JComponent buildCharges() {
        ...
        double bankTotal = 0;
        for (MoneyMarketCharge ch : new MoneyMarketBorrowingDAO().getCharges(d.getId())) {
